...
class Oauth extends Base
{
    ...
    public function callback($type = null, $code = null)
    {
        ...
        $extend = null;
        if ($type == 'tencent') {
            $extend = ['openid' => input("openid"), 'openkey' => input("openkey")];
        }
    ...
    private function _bang_handle($user_info, $type, $token)
    {
        ...
        $find_oauth_user = Db::name('OauthUser')->where(["oauth_from" => $type, $oauth_id => $token[$oauth_id]])->find();
        $need_bang = true;
        if ($find_oauth_user) {
    ...
    private function _login_handle($user_info, $type, $token)
    {
        $type = strtolower($type);
        $oauth_id = ($type == 'wechat' || $type == 'weixin') ? 'unionid' : 'openid';
        if ($oauth_id == 'unionid' && trim($user_info[$oauth_id]) === '') {
            $oauth_id = 'openid';
        }
        $find_oauth_user = Db::name('OauthUser')->where(["oauth_from" => $type, $oauth_id => $user_info[$oauth_id]])->find();
        $need_register = true;
        if ($find_oauth_user) {
            //更新第三方表
            $data = [
            ...
            Db::name('OauthUser')->where(["oauth_from" => $type, $oauth_id => $user_info[$oauth_id]])->update($data);
            $find_user = Db::name("member_list")->where(["member_list_id" => $find_oauth_user['uid']])->find();
            if ($find_user) {
                $need_register = false;
                ...
                Db::name("member_list")->where(['member_list_id' => $find_user["member_list_id"]])->update($data);
        ...
        if ($need_register) {
            ...
            if ($new_user_id) {
                ...
                if (!isset($new_oauth_user_data['openid'])) {
                    $new_oauth_user_data['openid'] = $user_info['openid'];
                }
                $new_oauth_user_id = Db::name("OauthUser")->insertGetId($new_oauth_user_data);
                if ($new_oauth_user_id) {
                    ...
                    $users_model->where(["member_list_id" => $new_user_id])->delete();
