...
class Options extends Model
{
     ...
     * 前台模板文件数组
     */
    public static function tpls($lang = 'zh-cn', $type = 'list', $folder = '')
    {
        $cacheKey = 'tpls_' . $lang . '_' . $type . '_' . $folder;
        $tpls = cache($cacheKey);
        if (empty($tpls)) {
            $sys = self::get_options('site_options', $lang);
            $arr = list_file(APP_PATH . 'home/view/' . $sys['site_tpl'] . '/' . $folder, '*.html');
            $tpls = [];
            foreach ($arr as $v) {
                $basename = basename($v['filename'], '.html');
                if (trim($type) != '') {
                    if (strpos($basename, $type) !== false && strpos($basename, 'ajax') === false) {
                        $tpls[] = $basename;
                    }
                }
            }
