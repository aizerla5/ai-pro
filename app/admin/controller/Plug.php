...
class Plug extends Base
{
     ...
     * @return mixed
     */
    public function plug_sug_add()
    {
        $sugId = input('id', false);
        if ($sugId) {
            $sug = Db::name('plug_sug')->where(['plug_sug_id' => $sugId])->find();
            if ($sug) {
                $this->assign('sug', $sug);
            }
        }
     ...
     * 留言执行添加或者修改
     */
    public function plug_sug_save()
    {
        $sugId = input('plug_sug_id', false);
        ...
        $sug_data = [
            ...
            'member_id' => 1,
            'is_read' => 1,
            'is_shown' => intval(input('is_shown', 0))
        ];

        if ($sug_data['is_shown']) {
            $sug_data['shown_at'] = date('Y-m-d H:i:s');
        }

        $continue = input('continue', 0, 'intval');
        if ($sugId) {
            $sug_data['plug_sug_id'] = $sugId;
            Db::name('plug_sug')->update($sug_data);
            $this->success('留言修改成功，返回列表页', url('admin/Plug/plug_sug_list'));
        } else {
            Db::name('plug_sug')->insert($sug_data);
            if ($continue) {
                $this->success('留言添加成功，继续发布', url('admin/Plug/plug_sug_add'));
            } else {
                $this->success('留言添加成功，返回列表页', url('admin/Plug/plug_sug_list'));
            }
        }
    }

    public function plug_sug_shown()
    {
        $p = input('p');
        $sugId = input('id');
        $rst = Db::name('plug_sug')->update([
            'plug_sug_id' => $sugId,
            'is_shown' => 1,
            'shown_at' => date('Y-m-d H:i:s')
        ]);
        if ($rst !== false) {
            $this->success('推送成功', url('admin/Plug/plug_sug_list', ['p' => $p]));
        } else {
            $this->error('推送失败', url('admin/Plug/plug_sug_list', ['p' => $p]));
        }
    }
