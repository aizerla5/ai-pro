...
class Model extends Base
{
    ...
    public function model_state()
    {
        ...
        if ($status == 1) {
            ...
            Db::name('model')->where(['model_id' => $id])->setField($statedata);
            $this->success('状态禁止');
        } else {
    ...
    public function model_runadd()
    {
        ...
        switch (config('database.type')) {
            case 'mysql' :
                ...
                foreach ($model_fields as $f => $fi) {
                    ...
                    switch ($fi ['type']) {
                        ...
                        case 'date' :
                            ...
                            if (in_array('required', $rules)) {
                                $fnull = 'NOT NULL';
                                $fi['default'] = isset($fi['default']) ? $fi['default'] : '';
                            }
                            $sql_fields[] = "`$f` DATE $fnull " . ($fi['default'] ? "DEFAULT $fi[default]" : "") . " COMMENT '$fi[title]'";
                            break;
                        case 'datetime':
                            $fnull = '';
                            if (in_array('required', $rules)) {
                            ...
                            $sql_fields[] = "`$f` DATETIME $fnull " . ($fi['default'] ? "DEFAULT $fi[default]" : "") . " COMMENT '$fi[title]'";
                            break;
                        //整数型
                        case 'number' :
                        case 'selectnumber' :
                ...
                //索引数组
                $model_indexes = input('model_indexes', '') ? explode(',', input('model_indexes', '')) : [];
    ...
    public function cmsrunedit()
    {
        ...
        $model_pkid = input($this->cms_pk, 0);
        $postdata = $this->handle_postdata($model_pkid, $this->cms_fields_edit);
    ...
    public function cmsimage_del()
    {
        $id = input('id', 0);
        $field = input('field');
        $result = false;
        if (!$this->cms_init($id, true)) {
            $this->error('不存在的模型');
        }
        $model = Db::name($this->cms_table)->where($this->cms_pk, $id)->field($field)->find();
        $k = request()->post('key', false);
        if ($model && $k) {
            $k--;
            $images = explode(',', $model[$field]);
            if (isset($images[$k]) && file_exists(ROOT_PATH . ltrim('/', $images[$k]))) {
                unset($images[$k]);
                @unlink(ROOT_PATH . ltrim('/', $images[$k]));
            }
            $images = array_merge($images, []);
            Db::name($this->cms_table)->update([
                $this->cms_pk => $id,
                $field => implode(',', $images)
            ]);
            $result = true;
        }
        echo $result;
    ...
    protected function cms_init($model_id, $is_edit = false, $is_search = false, $is_list = false)
    {
        ...
        if ($model) {
            ...
            $this->cms_fields[$model['model_cid']] = [
                ...
                'rules' => '',
                'default' => ''];
            $this->cms_fields = array_merge($this->cms_fields, $model['model_fields'] ? json_decode($model['model_fields'], true) : []);
            //处理baidu_map
            $fields = [];
            ...
            if ($is_edit) {
                $this->cms_fields_edit = $model['model_edit'] ? explode(',', $model['model_edit']) : [];
            ...
            if ($is_search) {
                $this->cms_fields_search = $model['search_list'] ? explode(',', $model['search_list']) : [];
            ...
            if ($is_list) {
                $this->cms_fields_list = $model['model_list'] ? explode(',', $model['model_list']) : [];
            ...
            if (!$is_edit || !in_array('readonly', $rules)) {
                switch ($f['type']) {
                    case 'images':
                        ...
                        if ($is_add) {
                            $postdata[$k] = implode(',', $allFiles);
                        } else if ($is_edit) {
                            $model = Db::name($this->cms_table)->where($this->cms_pk, $model_pkid)->field($k)->find();
                            $oldFiles = [];
                            if (trim($model[$k]) != '') {
                                $oldFiles = explode(',', $model[$k]);
                            }
                            $postdata[$k] = implode(',', array_merge($allFiles, $oldFiles));
                        }
                        break;
                    case 'files':
