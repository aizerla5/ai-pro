...
class News extends Base
{
    ...
    public function news_runadd()
    {
        ...
        $sl_data = [
            ...
            'news_pic_allurl' => implode(',', $allFiles),//多图路径
    ...
    public function news_edit()
    {
        ...
        if ($news_list['news_pic_allurl'] != '') {
            $files = [];
            $filesName = explode(',', $news_list['news_pic_allurl']);
    ...
    public function news_pic_sort()
    {
        $id = input('id');
        $model = Db::name('news')->where('n_id', $id)->find();
        if ($model) {
            ...
            Db::name('news')->update([
                'n_id' => $id,
                'news_pic_allurl' => implode(',', $images)
            ]);
        }
        exit;
    }

    public function news_pic_delete()
    {
        $id = input('id');
        $model = Db::name('news')->where('n_id', $id)->find();
        ...
        if ($model && $k) {
            $k--;
            $images = explode(',', $model['news_pic_allurl']);
    ...
    public function news_runedit()
    {
        ...
        $newId = input('n_id', 0);
        if ($newId) {
            $newModel = Db::name('news')->where(['n_id' => $newId])->find();
        }
        if (!request()->isAjax() && !$newModel) {
            $this->error('提交方式不正确', url('admin/News/news_list'));
        }
        $img_one = '';
        $allFiles = [];
        ...
        // 检测单图
        if (!$file) {
            $img_one = $newModel['news_img'];
        }
        // 检测多图
        if ($newModel) {
            $oldFiles = [];
            if (trim($newModel['news_pic_allurl']) != '') {
                $oldFiles = explode(',', $newModel['news_pic_allurl']);
            }
        ...
        $sl_data = [
            ...
            'news_img' => $img_one,
            'news_pic_type' => input('news_pic_type'),
            'news_pic_allurl' => implode(',', $allFiles),
            'news_pic_content' => input('news_pic_content', ''),
            'news_open' => input('news_open', 0),
            'news_scontent' => input('news_scontent', ''),
            'news_content' => htmlspecialchars_decode(input('news_content')),
