...
class Menu extends Base
{
    ...
    public function news_menu_add()
    {
        ...
        $layoutTpl = $options_model->tpls($this->lang, 'column', Config::get('template.layout_path'));
        $this->assign('layoutTpl', $layoutTpl);

        $listTpl = $options_model->tpls($this->lang);
        $this->assign('listTpl', $listTpl);

        $singleTpl = $options_model->tpls($this->lang, 'single');
        $this->assign('singleTpl', $singleTpl);
        return $this->fetch();
    }
    ...
    public function news_menu_runadd()
    {
        ...
        if (!request()->isAjax()) {
            ...
            $data = array(
                ...
                'parentid' => $parentid,
                'menu_layouttpl' => input('menu_layouttpl'),
