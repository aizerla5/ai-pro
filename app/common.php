 ...
 * @return string
 */
function go_curl($url, $type, $data = false, &$err_msg = null, $timeout = 20, $cert_info = [])
{
...
function export2excel($table, $file = '', $fields = '', $field_titles = '', $tag = '')
{
    ...
    $field_titles = empty($field_titles) ? [] : explode(",", $field_titles);
    if (empty($fields)) {
        $fields = $fieldsall;
        //成员数不一致,则取字段名为标题
...
{
    $navigation = cache("site_nav");
    if (empty($navigation)) {
        $navigation = get_menu_datas();
    }
    $tree = new \Tree();
    $tree->init($navigation);
 ...
 * @return string
 */
function get_tmenu($id = 0, $classname = 'navItem', $excludeUid = '')
{
    $navigation = cache("site_nav");
    if (empty($navigation)) {
        $navigation = get_menu_datas();
    ...
    $tree = new \Tree();
    $tree->init($navigation);
    return $tree->get_treeview_tmenu($id, $classname, $excludeUid);
}
...
{
    $navigation = Db::name("menu")->where('menu_l', Lang::detect())->where(array('menu_open' => 1))->order(array("listorder" => "ASC"))->select();
    foreach ($navigation as $key => $nav) {
        if ($nav['menu_type'] == 2) {
            $nav['href'] = $nav['menu_address'];
            if (intval($nav['menu_address'])) {
                $nav['href'] = url('home/Page/index?id=' . intval($nav['menu_address']));
            }
        } elseif ($nav['menu_type'] == 4) {
            //为了匹配单页路由
            $nav['href'] = url('home/Page/index?id=' . $nav['id']);
        } else {
            $nav['href'] = url('home/Page/index', array('id' => $nav['id']));
 ...
 * @return array|mixed
 */
function get_news($tag, $ispage = false, $pagesize = 10, $type = null, $v = null, $where = [], $currentPage = null)
{
 ...
 * @return array|mixed
 */
function get_comments($tag = "field:*;limit:0,5;order:createtime desc;", $where = [])
{
    $where = is_array($where) ? $where : [];
    $tag = param2array($tag);
    $field = !empty($tag['field']) ? $tag['field'] : '*';
...
function get_thumb($url, $width = 100, $height = 100, $avatar = 0)
{
    if (trim($url) != '') {
        ...
        if (file_exists($url) && is_numeric($width) && is_numeric($height)) {
            $basename = pathinfo($url);
            $thumbPath = $basename['dirname'] . "/{$width}x{$height}/";
            if (!is_dir($thumbPath)) {
                mkdir($thumbPath, 0755, 1);
            }
            $thumbnail = $thumbPath . $basename['basename'];
            if (!file_exists($thumbnail)) {
                $image = \think\Image::open($url);
                $image->thumb($width, $height, 4)->save($thumbnail, null, 100);
            }
 ...
 * @return false|string
 */
function format_date($json, $format = 'Y-m-d')
{
    if (isJson($json)) {
        $object = json_decode($json, true);
        if (isset($object['showdate']) && $object['showdate'] != '') {
            return date($format, $object['showdate']);
        }
    }
    return '';
}

function get_keyword_links($keywords)
{
    if (trim($keywords) != '') {
...
function get_data($table, $join, $joinon, $ids, $cid, $field, $limit, $order, $where_str, $ispage, $pagesize, $key, $page = 0)
{
    ...
    if ($key) {
        if (stripos($key, ':') !== false) {
            ...
            $k = $key;
            if ($model) {
                $search_list = $model['search_list'] ? explode(',', $model['search_list']) : array_keys($model['model_fields'] ? json_decode($model['model_fields'], true) : []);
                if ($search_list) {
...
function menu_text($lang = 'zh-cn')
{
    ...
    if (empty($menu_text)) {
        ...
        $menu_text = Db::name('menu')->where('menu_type <> 4')->where($map)->order('menu_l desc,listorder')->select();
