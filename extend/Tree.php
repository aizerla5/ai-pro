...
class Tree
{
     ...
     * @return string
     */
    public function get_treeview_tmenu($myid, $classname = 'navItem', $excludeUid = '')
    {
        //取出子菜单数组
        $child = $this->get_child($myid);
        $this->str = '';
        if (is_array($child)) {
            foreach ($child as $id => $a) {
                if (in_array($a['id'], explode(",", $excludeUid))) continue;
                @extract($a);
                $this->str .= '<a href="' . $a['href'] . '" class="' . $classname . '">' . $a['menu_name'] . '</a>';
            }
        }
        return $this->str;
    }
